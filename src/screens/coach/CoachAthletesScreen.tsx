...
import { coachApi, type CoachLinkedAthlete } from '@/api/coach';
import type { CoachStackParamList } from '@/navigation/types';

type NavProp = NativeStackNavigationProp<CoachStackParamList>;
...
export default function CoachAthletesScreen() {
  ...
  const { user } = useAuth();
  const [athletes, setAthletes] = useState<CoachLinkedAthlete[]>([]);
  const [loading, setLoading] = useState(true);

  const load = useCallback(() => {
    if (!user?.id) return;
    setLoading(true);
    coachApi.getLinkedAthletes(user.id)
      .then(setAthletes)
      .catch(() => setAthletes([]))
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      {loading ? (
        ...
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyBody}>Ask an athlete to invite you from their Profile screen, then enter the code from your Profile tab.</Text>
        </View>
      ) : (
        <FlatList
          data={athletes}
